...
in
{
  options.services.cliproxyapi = {
    ...
    openFirewall = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether to open the firewall for CLIProxyAPI.";
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    users.users.${cfg.user} = lib.mkIf (cfg.user == "cliproxyapi") {
      isSystemUser = true;
      group = cfg.group;
      home = cfg.dataDir;
      createHome = true;
    ...
    users.groups.${cfg.group} = lib.mkIf (cfg.group == "cliproxyapi") { };
